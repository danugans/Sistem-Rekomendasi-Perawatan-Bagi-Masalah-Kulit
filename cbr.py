...
self.feature_cols = ["age","gender","skin_type","acne","blackheads","dryness","redness","dark_spots","aging"]
self._prepare_model()
...
# Age should be scaled (here simple normalization 0-1 based on 0-100)
age = df['age'].astype(float).clip(0,100).values.reshape(-1,1)/100.0
# One-hot encode gender and skin_type
cat = df[['gender','skin_type']].astype(str)
self.ohe = OneHotEncoder(sparse=False, handle_unknown='ignore')
cat_enc = self.ohe.fit_transform(cat)
# symptom columns already numeric (0/1)
symp = df[['acne','blackheads','dryness','redness','dark_spots','aging']].astype(float).values
...
symp = q_df[['acne','blackheads','dryness','redness','dark_spots','aging']].astype(float).values
...
# Default reuse: return the solution text from the retrieved case
